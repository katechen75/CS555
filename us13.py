		...
		for index1 in range(0, len(children_births)):
			for index2 in range(1, len(children_births)):
				...
				if timedelta.total_seconds()/(60*60*24) > 2 and timedelta.total_seconds()/(60*60*24) < 30 * 8:
					print("ERROR: FAMILY: US12: " + children_births[index1]["id"] + ": Birthday " + children_births[index1]['birthday'].strftime("%m-%d-%Y") + " occurs too close to birthday of " + children_births[index2]["id"])
	return ret